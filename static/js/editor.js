...
// CV Data
const cvId =
  document.querySelector("[data-cv-id]")?.dataset.cvId ||
...
function renderExperienceItems() {
  ...
  if (!cvData.experience || cvData.experience.length === 0) {
    container.innerHTML =
      '<p class="text-muted small">No experience added yet. Click "Add" to add your first experience.</p>';
    return;
  }
...
function renderEducationItems() {
  ...
  if (!cvData.education || cvData.education.length === 0) {
    container.innerHTML =
      '<p class="text-muted small">No education added yet. Click "Add" to add your first education.</p>';
    return;
  }

  container.innerHTML = cvData.education
    .map(
      (edu, index) => `
...
function renderLanguagesItems() {
  ...
  if (!cvData.languages || cvData.languages.length === 0) {
    container.innerHTML =
      '<p class="text-muted small">No languages added yet. Click "Add" to add your first language.</p>';
    return;
  }
...
function renderProjectsItems() {
  ...
  if (!cvData.projects || cvData.projects.length === 0) {
    container.innerHTML =
      '<p class="text-muted small">No projects added yet. Click "Add" to add your first project.</p>';
    return;
  }
...
function renderCV() {
  ...
  let experienceHTML = "";
  if (cvData.experience && cvData.experience.length > 0) {
    experienceHTML = cvData.experience
      .map(
        (exp) => `
            <div class="cv-item">
  ...
  let educationHTML = "";
  if (cvData.education && cvData.education.length > 0) {
    educationHTML = cvData.education
      .map(
        (edu) => `
            <div class="cv-item">
                <div class="cv-item-header">
  ...
  let languagesHTML = "";
  if (cvData.languages && cvData.languages.length > 0) {
    languagesHTML = cvData.languages
      .map(
        (lang) => `
