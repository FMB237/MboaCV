...
def create_admin_user():
    ...
    if not User.query.filter_by(email=admin_email).first():
        admin = User(
            email=admin_email,
            password_hash=generate_password_hash("Th@9Sand5unNy"),
            full_name="Admin",
            is_admin=True,
        )
        db.session.add(admin)
        db.session.commit()
        print(f"Admin user created: {admin_email} / password: Th@9Sand5unNy")
...
def dashboard():
    cvs = (
        CV.query.filter_by(user_id=current_user.id).order_by(CV.updated_at.desc()).all()
    )
...
def create_cv():
    ...
    cv = CV(user_id=current_user.id, title=title, template=template)
...
def edit_cv(cv_id):
    ...
    return render_template(
        "editor.html", cv=cv, content=cv.get_content(), templates=templates
    )
...
def preview_cv(cv_id):
    ...
    return render_template(
        f"templates/{cv.template}.html", cv=cv, content=content, preview=True
    )
...
def upload_photo(cv_id):
    ...
    if file and allowed_file(file.filename):
        ...
        user_folder = os.path.join(
            app.config["UPLOAD_FOLDER"], f"user_{current_user.id}"
        )
        ...
        if content.get("personal", {}).get("photo"):
            old_photo = os.path.join(
                app.root_path, content["personal"]["photo"].lstrip("/")
            )
        ...
        content["personal"]["photo"] = (
            f"/static/uploads/user_{current_user.id}/{filename}"
        )
        cv.set_content(content)
        db.session.commit()

        return jsonify({"success": True, "photo_url": content["personal"]["photo"]})
